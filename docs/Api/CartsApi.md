...
$product_attribute_id = 789; // int | Id of the product attribute, required to add product to cart it product is not a subscription
...
try {
    $api_instance->deleteProductFromCart($cart_id, $product_id);
} catch (Exception $e) {
...
$api_instance = new Kaemo\Client\Api\CartsApi();
$cart_id = "cart_id_example"; // string | Cart id
